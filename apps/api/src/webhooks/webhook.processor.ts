...
export class WebhookWorker implements OnModuleInit, OnModuleDestroy {
  ...
  readonly queue = new Queue<WebhookJob>(WEBHOOK_QUEUE, {
  ...
  onModuleInit() {
    ...
    this.worker.on('failed', (job, err) => {
      this.logger.warn(
        `webhook delivery failed jobId=${job?.id} err=${err.message}`,
      );
    });
  ...
  private async deliver(job: Job<WebhookJob>): Promise<void> {
    ...
    if (!success) {
      ...
      if (disabled) {
        this.logger.warn(
          `Webhook ${webhookId} disabled after ${MAX_CONSECUTIVE_FAILS} consecutive failures`,
        );
      }
      throw new Error(
        `Delivery failed with status ${responseStatus ?? 'network error'}`,
      );
    }
